...
class Destination:
    ...
    def render(self) -> str:
        ...
        return f"[{self.label}]{via}(score {self.score:.2f})"
...
class Memory:
    ...
    def land(
        ...
        *,
        ...
        if mark_access:
            ...
            node.breadth.last_accessed = node.updated_at = node.breadth.last_accessed or ""
            from .model import now_iso
            node.breadth.last_accessed = now_iso()
            self.store.save_node(node)
        payload = self._payload(node, include_edges=include_edges)
        return payload
    ...
    def lca_context(self, refs: list[str]) -> dict:
        ...
        if common is None:
            for depth in range(min(len(c) for c in chains)):
                candidates = {c[depth] for c in chains}
                if len(candidates) == 1:
    ...
    def _payload(self, node: Node, *, include_edges: bool = False) -> Payload:
        ...
        total = len(deduped)
        payload = Payload(node=node, signpost=deduped[:TOP_K], total_destinations=total)
        if include_edges:
